...
export function Settings() {
  ...
  const load = useCallback(() => {
    ...
    Promise.all([operationsApi.policies(), operationsApi.departments()])
      .then(([policyResponse, departmentResponse]) => { setPolicies(policyResponse.data); setDepartments(departmentResponse.data.map((item) => item.name)); })
      .catch((requestError) => setError(errorMessage(requestError, "Không thể tải cài đặt.")))
  ...
  return <section className="section-stack settings-page content-readable">
    <Toolbar eyebrow="Cài đặt vận hành" title="Chính sách và danh mục" description="Các thay đổi dưới đây được áp dụng ngay cho tài khoản và luồng mượn mới." actions={<button className="primary" onClick={() => void save()}>Lưu thay đổi</button>} />
    {loading ? <PageLoader /> : error ? <PageError message={error} onRetry={() => void load()} /> : <>
