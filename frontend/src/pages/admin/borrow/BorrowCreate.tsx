...
export function BorrowCreate() {
  ...
  const load = useCallback(() => {
    ...
    Promise.all([userApi.list({ limit: 100 }), bookApi.copies({ limit: 100, status: "available" })])
      .then(([u, c]) => { setUsers(u.data.data); setCopies(c.data.data); })
      .catch((error) => setLoadError(errorMessage(error, "Không thể tải dữ liệu tạo phiếu mượn")))
  ...
  return <section className="borrow-create section-stack"><Toolbar eyebrow="Lưu thông thư viện" title="Tạo phiếu mượn" description="Chọn người dùng hiện có hoặc ghi nhận độc giả vãng lai ngay tại quầy." />
    {loading ? <PageLoader /> : loadError ? <PageError message={loadError} onRetry={() => void load()} /> : <><div className="segmented" role="tablist"><button className={mode === "existing" ? "selected" : ""} onClick={() => setMode("existing")}><Users size={17} />Người dùng hiện có</button><button className={mode === "visitor" ? "selected" : ""} onClick={() => setMode("visitor")}><UserPlus size={17} />Độc giả vãng lai</button></div>
