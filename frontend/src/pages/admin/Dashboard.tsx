...
export function Dashboard() {
  ...
  const load = useCallback(() => {
    ...
    Promise.all([
      ...
      borrowApi.requests({ status: "pending", limit: 5 }),
    ])
      .then(([dashboardResponse, requestResponse]) => {
  ...
  return (
    <div className="section-stack admin-dashboard">
      <Toolbar
        title="Tổng quan"
        description="Các việc cần xử lý trong ngày hôm nay."
      ...
      <section className="metric-grid" aria-label="Chỉ số thư viện">
        ...
        <MetricCard
          ...
          detail="Bản sách đang lưu thông"
          onClick={() => navigate("/admin/borrow")}
        />
        <MetricCard
          label="Sự cố mở"
          value={stats?.openIncidents || 0}
          detail="Mất hoặc hỏng"
      ...
      <section className="dashboard-grid">
        <section className="panel admin-request-panel">
          <div className="panel-heading">
            <div>
              ...
              <p className="muted">
                Các yêu cầu mượn đang chờ thư viện phản hồi.
              </p>
            </div>
            <button
          ...
          {requests.length ? (
            <div className="activity-list">
              {requests.map((request) => (
                <button
                  className="activity-row request-row"
                  type="button"
                  key={request.id}
                  onClick={() => navigate("/admin/borrow-requests")}
                >
                  ...
                  <div>
                    ...
                    <span>{request.full_name || "Độc giả"}</span>
                  </div>
        ...
        <aside className="panel attention admin-reminders">
          <div className="panel-heading">
            <div>
              <h2>Nhắc việc</h2>
              <p className="muted">Các điểm cần chú ý.</p>
            </div>
          </div>
          <button
          ...
          >
            ...
            <span>Sự cố sách</span>
            <strong>{stats?.openIncidents || 0}</strong>
          </button>
        </aside>
      </section>
      <section className="panel table-panel recent-borrow-panel">
        <div className="panel-heading">
          <div>
            <h2>Hoạt động mượn gần đây</h2>
          </div>
        </div>
        {data?.recent?.length ? (
          <table>
          ...
          <EmptyState
            ...
            text="Các phiếu mới sẽ xuất hiện tại đây."
          />
        )}
      </section>
    </div>
