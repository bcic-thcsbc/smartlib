...
export function Profile() {
  ...
  const load = useCallback(() => {
    ...
    Promise.all([userApi.profile(), operationsApi.departments()])
      ...
      .catch((requestError) =>
        setError(errorMessage(requestError, "Không thể tải hồ sơ.")),
      );
  }, []);
  useEffect(() => {
    load();
  ...
  const save = async (event: React.FormEvent) => {
    ...
    if (
      ...
      (profile.user_type === "student" &&
        !validClassName(profile.class_name || ""))
    ) {
    ...
    try {
      await userApi.updateProfile({
        ...profile,
        class_name: normalizeClassName(profile.class_name || ""),
      });
  ...
  return (
    <div className="section-stack profile-grid">
      <Toolbar
        title="Hồ sơ cá nhân"
        description="Cập nhật thông tin liên hệ và lớp hoặc tổ bộ môn."
      />
      <form className="panel profile-form" onSubmit={save}>
        ...
        <div className="form-grid">
          ...
          <label>
            ...
            />
            {profile.phone && !validPhone(profile.phone) && (
              <small className="field-error">Số điện thoại không hợp lệ.</small>
            )}
          </label>
          <label>
            <FieldLabel>
